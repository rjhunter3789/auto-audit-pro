...
async function runComprehensiveAudit(url, $, driver = null) {
    ...
    // Run all test categories IN PARALLEL for speed
    console.log('[AUDIT] Running all tests in parallel for faster results...');
    const startTime = Date.now();
    
    const [connectivity, performance, seo, ux, content, technical, brandCompliance, leadGen] = 
        await Promise.all([
            testBasicConnectivity(url, $).catch(err => {
                console.error('[AUDIT] Connectivity test error:', err.message);
                return { score: 0, maxScore: 5, tests: [] };
            }),
            testPerformance($, null, url).catch(err => {
                console.error('[AUDIT] Performance test error:', err.message);
                return { score: 0, maxScore: 5, tests: [] };
            }),
            testSEO($, url).catch(err => {
                console.error('[AUDIT] SEO test error:', err.message);
                return { score: 0, maxScore: 5, tests: [] };
            }),
            testUserExperience($, url).catch(err => {
                console.error('[AUDIT] UX test error:', err.message);
                return { score: 0, maxScore: 5, tests: [] };
            }),
            testContent($, brand, url).catch(err => {
                console.error('[AUDIT] Content test error:', err.message);
                return { score: 0, maxScore: 5, tests: [] };
            }),
            testTechnical($, url).catch(err => {
                console.error('[AUDIT] Technical test error:', err.message);
                return { score: 0, maxScore: 5, tests: [] };
            }),
            testBrandCompliance($, brand, url).catch(err => {
                console.error('[AUDIT] Brand compliance test error:', err.message);
                return { score: 0, maxScore: 5, tests: [] };
            }),
            testLeadGeneration($, url).catch(err => {
                console.error('[AUDIT] Lead generation test error:', err.message);
                return { score: 0, maxScore: 5, tests: [] };
            })
        ]);
    
    const parallelTime = Date.now() - startTime;
    console.log(`[AUDIT] All tests completed in ${parallelTime}ms (parallel execution)`);
    ...
    categoryResults.forEach((cat, index) => {
        ...
        results.categories.push({
            ...
            testsRun: cat.tests.length,
            testsTotal: cat.tests.length,
            weight: weight
        });
...
async function runSEOAudit(url, $) {
    ...
    results.categories.push({
        ...
        testsRun: seo.tests.length,
        testsTotal: seo.tests.length
    });
