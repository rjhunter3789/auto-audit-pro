...
async function testSEO($, url) {
    ...
    if (title.length > 0 && title.length <= 60) {
        ...
        issues.push({
            title: hasOGTitle ? 'Missing HTML title tag' : 'Missing title tag',
            details: hasOGTitle 
                ? 'Social media title tag found but no standard HTML title tag. The <title> tag is critical for SEO. Your CMS may be misconfigured.'
                : 'No title tag detected. Title tags may be added dynamically by JavaScript. Verify using View Source. Add a 50-60 character title for better SEO.',
            priority: 'high',
    ...
    if (metaDesc.length > 0 && metaDesc.length <= 160) {
        ...
        if (hasOGDescription || hasTwitterDescription) {
            score += 0.5;
            issues.push({
                title: 'Missing standard meta description',
                details: 'Social media meta descriptions found but no standard meta description tag. While social tags are good, search engines primarily use the standard meta description tag.',
                priority: 'medium',
                category: 'SEO Analysis - Meta Tags',
                confidence: 'high'
            });
            ...
            issues.push({
                ...
                details: 'No meta description detected. Meta descriptions may be added dynamically by your CMS. Use Google Search Console to verify. If missing, add a compelling 150-160 character description.',
                priority: 'high',
                category: 'SEO Analysis - Meta Tags',
                confidence: 'moderate'
            });
...
async function testUserExperience($) {
    ...
    if (hasTelLinks || hasLabeledPhone || hasPhoneInHeader || hasCallNow || (hasPhoneAnywhere && hasAddress)) {
        ...
        issues.push({
            ...
            details: 'No phone number detected on the page. This is unusual for a dealership website and may indicate a technical issue or that contact information is loaded dynamically.',
            priority: 'high',
            category: 'User Experience - Contact',
            confidence: 'manual-review'
        });
...
async function testLeadGeneration($) {
    ...
    if (socialLinks.length > 0) {
        ...
        issues.push({
            title: 'Social media links not found',
            details: 'No social media links detected. Social links may be in the footer or loaded dynamically. Adding social media links helps build trust and provides additional communication channels.',
            priority: 'low',
