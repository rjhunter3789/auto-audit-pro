...
async function testLeadGeneration($) {
    ...
    const phoneNumbers = pageText.match(phonePattern) || [];
    
    tests.push({ name: 'Clickable Phone Links', value: phoneLinks.length });
    tests.push({ name: 'Phone Numbers Found', value: phoneNumbers.length });
    
    if (phoneLinks.length > 0) {
        score += 1;
    } else if (phoneNumbers.length > 0) {
        // Phone numbers exist but aren't clickable
        score += 0.5; // Partial credit
        issues.push({
            title: 'Make phone numbers clickable',
            details: `Found ${phoneNumbers.length} phone number(s) that should be clickable. Add tel: links (e.g., <a href="tel:+1234567890">123-456-7890</a>) to enable mobile users to tap-to-call.`,
            priority: 'medium',
