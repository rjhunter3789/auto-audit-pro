...
async function testUserExperience($) {
    ...
    // Also check for "Call now" or "Call today" patterns
    ...
    const hasPhoneInHeader = phonePattern.test(headerText);
    phonePattern.lastIndex = 0;
    const hasPhoneAnywhere = phonePattern.test(bodyText);
    ...
    if (hasPhoneInHeader || hasCallNow || (hasPhoneAnywhere && hasAddress)) {
        score += 1;
...
async function testLeadGeneration($) {
    ...
    const phonePattern = /(\+?1[-.]?\s?)?(\(?\d{3}\)?[-.]?\s?\d{3}[-.]?\s?\d{4})/g;
    // Check header first, then full page
    const headerText = $('header, .header, #header, [class*="header"], [id*="header"], body > div:first-child, body > div:nth-child(2)').text();
    const pageText = $('body').text();
    
    const headerPhones = headerText.match(phonePattern) || [];
    const allPhones = pageText.match(phonePattern) || [];
    ...
    if (phoneLinks.length > 0) {
        score += 1;
    } else if (headerPhones.length > 0) {
        // Phone numbers in header but not clickable - still good for visibility
        score += 0.8;
        issues.push({
            title: 'Make phone numbers clickable',
            details: `Found ${headerPhones.length} phone number(s) in header that should be clickable. Add tel: links to enable tap-to-call on mobile devices.`,
            priority: 'low',
        ...
        issues.push({
            ...
            details: 'Add a visible phone number with tel: link for easy contact. Phone numbers are crucial for dealership lead generation.',
            priority: 'high',
            category: 'Lead Generation - Contact'
        });
    }
