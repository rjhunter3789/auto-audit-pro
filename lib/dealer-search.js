    ...
    getSearchSummary(searchResults) {
        ...
        if (metadata) {
            return {
                ...
                coverage: metadata.states ? `Locations in ${metadata.states.length} states${metadata.states.length > 3 ? ` including ${metadata.states.slice(0, 3).join(', ')}` : `: ${metadata.states.join(', ')}`}` : 'Multiple states',
