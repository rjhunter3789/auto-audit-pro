...
async function auditVDP($, url) {
    ...
    if (hasGallery || hasThumbnails) {
        ...
        if (largeImages.length < 5) {
            ...
            issues.push({
                title: `Image Gallery Review ${fullPageContext}`,
                details: `Gallery system detected ${fullPageContext}. Images may load on-demand or via user interaction. Manual verification recommended to confirm image count meets the 20+ best practice standard.`,
                priority: 'info',
            ...
            issues.push({
                title: `Image Gallery Enhancement Opportunity ${fullPageContext}`,
                details: `Gallery system functioning well ${fullPageContext}. Currently showing ${largeImages.length} images. Consider adding more angles and detail shots to reach the 20-40 image best practice range.`,
                priority: 'low',
        ...
        issues.push({
            title: `Vehicle Image System Not Detected ${fullPageContext}`,
            details: `No image gallery system or vehicle images detected ${fullPageContext}. This may indicate a technical issue or that images are loaded through a method not detected by automated scanning.`,
            priority: 'high',
        ...
        score += 0.4;
        issues.push({
            title: `Basic Image Display Detected ${fullPageContext}`,
            details: `${largeImages.length} vehicle images found without a gallery system ${fullPageContext}. Implementing a proper image gallery with 20+ photos including multiple angles, interior views, and feature highlights is recommended for optimal user experience.`,
            priority: 'medium',
            category: 'Content - VDP',
            confidence: 'high'
        });
        ...
        if (largeImages.length < 20) {
            issues.push({
                title: `Image Collection Enhancement ${fullPageContext}`,
                details: `Good image coverage with ${largeImages.length} photos ${fullPageContext}. Adding ${20 - largeImages.length} more images would meet the industry best practice of 20+ photos per vehicle.`,
                priority: 'low',
                category: 'Content - VDP',
                confidence: 'high'
            });
    ...
    if (!hasPrice) {
        issues.push({
            title: `No pricing information found ${fullPageContext}`,
            details: `Vehicle pricing is critical for transparency ${fullPageContext}. Display MSRP, sale price, and any applicable discounts clearly.`,
            priority: 'high',
            category: 'Content - VDP'
        });
    } else {
        ...
        if (!hasDisclaimer) {
            issues.push({
                title: `Missing pricing disclaimer ${fullPageContext}`,
                details: `Add disclaimer about taxes, fees, and other charges to ensure compliance and transparency ${fullPageContext}.`,
                priority: 'medium',
    ...
    if (specCount < 3) {
        issues.push({
            title: `Limited vehicle specifications ${fullPageContext}`,
            details: `Add comprehensive vehicle specs including engine, transmission, MPG, features, and options ${fullPageContext}.`,
            priority: 'medium',
            category: 'Content - VDP'
        });
    } else {
    ...
    if (ctaButtons.length === 0) {
        issues.push({
            title: `No clear CTAs ${fullPageContext}`,
            details: `Add prominent buttons for financing, test drive scheduling, and contact ${fullPageContext}. Multiple CTAs increase conversion.`,
            priority: 'high',
            category: 'Lead Generation - VDP'
        });
    } else if (ctaButtons.length < 3) {
        issues.push({
            title: `Limited CTAs ${fullPageContext}`,
            details: `Consider adding more action buttons ${fullPageContext}: Calculate Payment, Schedule Test Drive, Check Availability, Get ePrice.`,
            priority: 'medium',
            category: 'Lead Generation - VDP'
        });
        score += 0.5;
    ...
    if (!hasCalculator) {
        issues.push({
            title: `No payment calculator found ${fullPageContext}`,
            details: `Add an interactive payment calculator ${fullPageContext} to help customers understand affordability. This significantly increases lead quality.`,
            priority: 'high',
            category: 'Tools - VDP'
        });
    } else {
    ...
    if (isUsed) {
        ...
        if (!hasCarfax) {
            issues.push({
                title: `No vehicle history report ${fullPageContext}`,
                details: `Provide CARFAX or AutoCheck reports for used vehicles ${fullPageContext} to build trust and transparency.`,
                priority: 'high',
                category: 'Trust - VDP'
            });
        } else {
...
async function auditServicePage($, url) {
    ...
    if (!hasScheduling && schedulingButtons.length === 0) {
        issues.push({
            title: `No online service scheduling ${fullPageContext}`,
            details: `Add online appointment scheduling ${fullPageContext} to reduce phone calls and improve customer convenience. This is now expected by most customers.`,
            priority: 'high',
    ...
    if (!hasMenu) {
        issues.push({
            title: `No service menu found ${fullPageContext}`,
            details: `Display common services offered with descriptions ${fullPageContext}. Transparency builds trust and reduces friction.`,
            priority: 'medium',
        ...
        if (!hasPricing) {
            issues.push({
                title: `No service pricing displayed ${fullPageContext}`,
                details: `Consider showing pricing for common services like oil changes ${fullPageContext}. Price transparency increases appointments.`,
                priority: 'low',
                category: 'Content - Service'
            });
        } else {
    ...
    if (!hasSpecials) {
        issues.push({
            title: `No service specials displayed ${fullPageContext}`,
            details: `Promote current service specials and coupons prominently ${fullPageContext}. Deals drive immediate action.`,
            priority: 'medium',
    ...
    if (!hasHours) {
        issues.push({
            title: `Service hours not displayed ${fullPageContext}`,
            details: `Clearly display service department hours ${fullPageContext}, including Saturday availability if applicable.`,
            priority: 'high',
    ...
    if (!hasPhone) {
        issues.push({
            title: `No service department phone ${fullPageContext}`,
            details: `Display a direct service department phone number prominently ${fullPageContext} for customers who prefer to call.`,
            priority: 'medium',
    ...
    if (amenityCount === 0) {
        issues.push({
            title: `No service amenities mentioned ${fullPageContext}`,
            details: `Highlight amenities like shuttle service, loaners, WiFi, and comfortable waiting areas ${fullPageContext} to differentiate your service.`,
            priority: 'low',
            category: 'Marketing - Service'
        });
    } else {
...
async function auditInventoryPage($, url) {
    ...
    if (hasFilterUI || hasFacetedSearch) {
        ...
        if (totalFilterElements < 3) {
            ...
            issues.push({
                title: `Filter System Review ${fullPageContext}`,
                details: `Filter interface detected ${fullPageContext}. Filters may load dynamically or populate based on inventory. Manual verification recommended to confirm comprehensive filtering options are available.`,
                priority: 'info',
            ...
            issues.push({
                title: `Expand Filter Options ${fullPageContext}`,
                details: `Good foundation with ${totalFilterElements} filters found ${fullPageContext}. Consider adding more options like: ${
        ...
        issues.push({
            title: `Search Filter System Not Detected ${fullPageContext}`,
            details: `No inventory filtering system found ${fullPageContext}. This may indicate filters load after user interaction or through a separate search interface. Manual verification strongly recommended.`,
            priority: 'high',
        ...
        score += 0.4;
        issues.push({
            title: `Basic Search Functionality ${fullPageContext}`,
            details: `${totalFilterElements} filter options detected ${fullPageContext}. Modern inventory search typically includes 8-12 filter types: Year, Make, Model, Price Range, Mileage, Body Style, Color, Transmission, Fuel Type, and Features.`,
            priority: 'medium',
        ...
        if (totalFilterElements < 8) {
            issues.push({
                title: `Filter Enhancement Opportunity ${fullPageContext}`,
                details: `Solid filtering with ${totalFilterElements} options ${fullPageContext}. Adding ${8 - totalFilterElements} more filter types would match industry best practices.`,
                priority: 'low',
                category: 'Enhancement - Inventory',
                confidence: 'high'
            });
    ...
    if (vehicleCards.length === 0) {
        issues.push({
            title: `No vehicle results visible ${fullPageContext}`,
            details: `Ensure inventory displays properly with clear vehicle cards showing key information ${fullPageContext}.`,
            priority: 'high',
        ...
        if (!hasGrid) {
            issues.push({
                title: `Poor inventory layout ${fullPageContext}`,
                details: `Use a responsive grid layout for inventory display ${fullPageContext}. Allow users to switch between grid and list views.`,
                priority: 'medium',
    ...
    if (!hasSorting) {
        issues.push({
            title: `No sorting options ${fullPageContext}`,
            details: `Add sorting by Price, Year, Mileage, and Date Listed ${fullPageContext}. Sorting helps users find vehicles faster.`,
            priority: 'medium',
            category: 'Functionality - Inventory'
        });
    } else {
    ...
    if (infoFound < 3) {
        issues.push({
            title: `Insufficient quick view information ${fullPageContext}`,
            details: `Display Price, Year, Make/Model, Mileage, and Key Features on inventory cards ${fullPageContext} for quick scanning.`,
            priority: 'medium',
