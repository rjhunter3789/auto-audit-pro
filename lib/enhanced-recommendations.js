...
// Import ROI configuration
const { getROIConfig } = require('./roi-config');

// Calculate potential ROI using admin-configurable values
function calculatePotentialROI(recommendations) {
    ...
    // Use admin-configured values for calculation
    const avgLeadsPerMonth = config.avgLeadsPerMonth;
    const avgConversionRate = config.avgConversionRate;
    const avgDealValue = config.avgDealValue;
    const avgGrossProfit = config.avgGrossProfit;
    ...
    // Use gross profit instead of deal value for more accurate ROI
    improvements.estimatedValue = Math.round(additionalConversions * avgGrossProfit * 12); // Annual value
