...
class MonitoringScheduler {
    ...
    async loadMonitoringProfiles() {
        try {
            const query = 'SELECT * FROM monitoring_profiles WHERE monitoring_enabled = true';
    ...
    async processAlerts(profile, results, alerts) {
        for (const alert of alerts) {
            try {
                ...
                if (recentAlert) {
                    console.log(`[Scheduler] Skipping duplicate alert for ${profile.dealer_name}`);
                    continue;
                }
                ...
                if (alert.alert_level === 'RED' || profile.alert_preferences.email) {
    ...
    /**
     * Check if similar alert was sent recently (within 1 hour)
     */
    async checkRecentAlert(profileId, alert) {
        try {
            ...
            const result = await this.dbPool.query(query, [profileId, alert.alert_type]);
