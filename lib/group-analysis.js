...
async function testLocationDirectory($, url, dealerLinks = null) {
    ...
    if (dealerLinks && dealerLinks.length > 0) {
        ...
        if (uniqueCount >= 10) {
            return {
                test: { name: 'Location Directory', value: `${uniqueCount} dealer locations found` },
                score: 2,
            ...
            return {
                test: { name: 'Location Directory', value: `Only ${uniqueCount} dealer locations found` },
                score: 1,
                issue: {
...
async function extractDealerLinks($, driver, baseUrl) {
    ...
    // First, try using the location crawler for comprehensive discovery
...
function isDealerLink(href, text, baseUrl) {
    ...
    // Debug: Log some rejected links to see what we're missing
    const debugRejected = false; // Set to true for debugging
    ...
    // Skip only very generic text
    if (textLower === 'click here' || textLower === 'read more' || 
        textLower === 'home' || textLower === '') {
        return false;
    }
    
    // For external dealer sites (like Kendall)
    if (href.includes('http') && !href.includes(baseUrl)) {
        // Check if it looks like a dealer website
        const dealerDomainPatterns = [
            /[a-z]+ford[a-z]*\.com/i,
            /[a-z]+chevrolet[a-z]*\.com/i,
            /[a-z]+toyota[a-z]*\.com/i,
            /[a-z]+honda[a-z]*\.com/i,
            /[a-z]+nissan[a-z]*\.com/i,
            /[a-z]+chrysler[a-z]*\.com/i,
            /[a-z]+jeep[a-z]*\.com/i,
            /[a-z]+gmc[a-z]*\.com/i,
            /[a-z]+hyundai[a-z]*\.com/i,
            /[a-z]+kia[a-z]*\.com/i,
            /[a-z]+mazda[a-z]*\.com/i,
            /[a-z]+subaru[a-z]*\.com/i,
            /[a-z]+volkswagen[a-z]*\.com/i,
            /[a-z]+volvo[a-z]*\.com/i,
            /[a-z]+mercedes[a-z]*\.com/i,
            /[a-z]+bmw[a-z]*\.com/i,
            /[a-z]+audi[a-z]*\.com/i,
            /[a-z]+lexus[a-z]*\.com/i,
            /[a-z]+acura[a-z]*\.com/i,
            /[a-z]+infiniti[a-z]*\.com/i,
            /[a-z]+lincoln[a-z]*\.com/i,
            /[a-z]+buick[a-z]*\.com/i,
            /[a-z]+cadillac[a-z]*\.com/i,
            /[a-z]+mitsubishi[a-z]*\.com/i,
            /[a-z]+ducati[a-z]*\.com/i,
            /[a-z]+harley[a-z]*\.com/i,
            /[a-z]+yamaha[a-z]*\.com/i,
            /[a-z]+kawasaki[a-z]*\.com/i,
            /[a-z]+suzuki[a-z]*\.com/i,
            /[a-z]+triumph[a-z]*\.com/i,
            /[a-z]+indian[a-z]*\.com/i,
            /[a-z]+ktm[a-z]*\.com/i,
            /[a-z]+motorcycle[a-z]*\.com/i,
            /[a-z]+powersports[a-z]*\.com/i,
            /[a-z]+usedcars[a-z]*\.com/i,
            /[a-z]+pre-?owned[a-z]*\.com/i,
            /[a-z]*commercial[a-z]*\.com/i,
            /[a-z]*fleet[a-z]*\.com/i,
            /[a-z]*truck[a-z]*\.com/i,
            /[a-z]*service[a-z]*\.com/i,
            /[a-z]*collision[a-z]*\.com/i,
            /[a-z]*bodyshop[a-z]*\.com/i,
            /[a-z]*parts[a-z]*\.com/i,
            /[a-z]*rv[a-z]*\.com/i,
            /[a-z]*marine[a-z]*\.com/i,
            /[a-z]*boat[a-z]*\.com/i,
            /commercial-vehicle.*\.kendallautogroup\.com/i,
            /.*\.geeautomotive\.com/i
        ];
        
        if (dealerDomainPatterns.some(pattern => pattern.test(href))) {
            return true;
        }
    }
    ...
    // Check if text contains dealer name patterns (expanded list including motorcycles)
    ...
    if (dealerNamePattern.test(text)) {
        return true;
    }
    
    // Check for "Used Car" locations
    const usedCarPattern = /\b(Used Car|Pre-Owned|Preowned|CPO|Certified)\s*(Center|Location|Store|Dealership|Outlet|Superstore)?\b/i;
    if (usedCarPattern.test(text)) {
        return true;
    }
    
    // Check for Commercial Service Centers and other specialty locations
    const commercialPattern = /\b(Commercial|Fleet|Work Truck)\s*(Service|Center|Vehicle|Sales|Department|Location)?\b/i;
    if (commercialPattern.test(text)) {
        return true;
    }
    
    // Check for Service Centers
    const servicePattern = /\b(Service Center|Quick Lube|Express Service|Collision Center|Body Shop|Parts Center|Repair Center)\b/i;
    if (servicePattern.test(text)) {
        return true;
    }
    
    // Check for other specialty dealerships
    const specialtyPattern = /\b(RV|Recreational Vehicle|Marine|Boat|Powersports|ATV|UTV|Side-by-Side|Motorcycle)\s*(Center|Sales|Location|Dealership|Store)?\b/i;
    if (specialtyPattern.test(text)) {
        return true;
    }
    
    // Pattern: "Brand of Location" or "Location Brand"
    const brandLocationPatterns = [
        /(Ford|Chevrolet|Chevy|Toyota|Honda|Nissan|Chrysler|Jeep|GMC|Hyundai|Kia|Mazda|Subaru|Volkswagen|VW|Audi|BMW|Mercedes|Ducati|Harley|Yamaha|Kawasaki|Suzuki|INEOS)\s+of\s+[A-Z]/i,
        /[A-Z][a-zA-Z\s]+\s+(Ford|Chevrolet|Chevy|Toyota|Honda|Nissan|Chrysler|Jeep|GMC|Hyundai|Kia|Mazda|Subaru|Volkswagen|VW|Audi|BMW|Mercedes|Ducati|Harley|Yamaha|Kawasaki|Suzuki|Motors|Automotive|Auto)$/i
    ];
    
    if (brandLocationPatterns.some(pattern => pattern.test(text))) {
        return true;
    }
    
    // Check if text looks like a location name with automotive context
    if ((textLower.includes('location') || textLower.includes('store') || 
         textLower.includes('dealership') || textLower.includes('showroom') ||
         textLower.includes('outlet') || textLower.includes('center') ||
         textLower.includes('superstore') || textLower.includes('motors') ||
         textLower.includes('automotive') || textLower.includes('auto group') ||
         textLower.includes('powersports') || textLower.includes('motorcycle') ||
         textLower.includes('commercial') || textLower.includes('fleet') ||
         textLower.includes('service') || textLower.includes('collision') ||
         textLower.includes('body shop') || textLower.includes('parts') ||
         textLower.includes('marine') || textLower.includes('rv')) &&
        text.length > 8 && text.length < 100) {
        return true;
    }
    
    // Special case for generic location names with numbers or cities
    const genericLocationPattern = /^(Gee|Kendall|AutoNation|Lithia|Group\s*1|Sonic)?\s*[\w\s]+(Location|Store|Center|Outlet|Plaza|Place)\s*\d*$/i;
    if (genericLocationPattern.test(text)) {
        return true;
    }
    
    // For links that have text mentioning city names or addresses
    const cityPatterns = /\b(North|South|East|West|Downtown|Uptown)\s+[A-Z][a-z]+\b/;
    const addressPatterns = /\b\d+\s+[A-Z][a-z]+\s+(Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Highway|Hwy|Drive|Dr)\b/i;
    
    if ((cityPatterns.test(text) || addressPatterns.test(text)) && 
        ALL_BRAND_VARIATIONS.some(brand => textLower.includes(brand.toLowerCase()))) {
        return true;
    }
