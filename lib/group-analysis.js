...
async function extractDealerLinks($, driver, baseUrl) {
    ...
    const selectors = [
        ...
        '.our-locations a',
        '.our-dealerships a',
        // Don't use overly broad selectors like a[href*=".com"]
    ];
...
function isDealerLink(href, text, baseUrl) {
    ...
    const skipPatterns = [
        ...
        // Skip generic pages
        '/privacy', '/terms', '/sitemap', '/contact', '/about',
        '/careers', '/employment', '/blog', '/news'
    ];
    ...
    // Skip if it's just "locations" or "view details"
    if (textLower === 'locations' || textLower === 'view details' || 
        textLower === 'learn more' || textLower === 'click here') {
        return false;
    }
    
    // For Kendall-style external dealer sites
    ...
    // For internal dealer pages
    if (href.includes('/dealer-info') || 
        (href.includes('/locations/') && href.split('/locations/')[1]?.length > 3)) {
        return true;
    }
    
    // Check if text contains dealer name patterns
    const dealerNamePattern = /^[A-Z][a-zA-Z\s&'-]+(Ford|Chevrolet|Toyota|Honda|Nissan|Chrysler|Jeep|GMC|Hyundai|Kia|Mazda|Subaru|Volkswagen|Audi|BMW|Mercedes)(\s|$)/i;
    if (dealerNamePattern.test(text)) {
        return true;
    }
    
    // Pattern: "Brand of Location"
    const brandOfLocationPattern = /(Ford|Chevrolet|Toyota|Honda|Nissan|Chrysler|Jeep|GMC|Hyundai|Kia)\s+of\s+[A-Z]/i;
    if (brandOfLocationPattern.test(text)) {
        return true;
    }
