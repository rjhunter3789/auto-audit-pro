...
function isDealerLink(href, text, baseUrl) {
    ...
    const skipPatterns = [
        ...
        // Skip ONLY manufacturer sites (exact domain match)
        'www.ford.com', 'ford.com/', 'www.chevrolet.com', 'chevrolet.com/',
        'www.toyota.com', 'toyota.com/', 'www.honda.com', 'honda.com/',
        'www.nissan.com', 'nissan.com/', 'www.chrysler.com', 'chrysler.com/',
        'www.jeep.com', 'jeep.com/', 'www.gmc.com', 'gmc.com/',
    ...
    // REMOVED dealer-info pattern - those are search results, not individual dealerships!
    if (href.includes('/dealership-info') ||
        href.includes('/location-detail') ||
        href.includes('/store-detail') ||
        (href.includes('/locations/') && href.split('/locations/')[1]?.length > 3 && !href.includes('?')) ||
        (href.includes('/dealers/') && href.split('/dealers/')[1]?.length > 3 && !href.includes('?')) ||
        (href.includes('/stores/') && href.split('/stores/')[1]?.length > 3 && !href.includes('?'))) {
        return true;
    }
    ...
    // Skip common UI elements and search/filter links
    if (textLower === 'view details' ||
        textLower === 'see details' ||
        textLower === 'more info' ||
        textLower === 'learn more' ||
        textLower === 'dealer info' ||
        textLower === 'get directions' ||
        textLower === 'view inventory' ||
        href.includes('?') && (href.includes('filter') || href.includes('search') || href.includes('sort'))) {
        return false;
    }
...
// New function to find and analyze location pages
async function findAndAnalyzeLocationPage($, baseUrl, driver = null) {
    const finder = new LocationFinder(baseUrl);
    
    // First, find the locations page
    const locationPage = finder.findLocationPage($);
    
    if (locationPage.url && locationPage.confidence >= 70) {
        console.log(`[LocationFinder] Found location page with ${locationPage.confidence}% confidence: ${locationPage.url}`);
        
        // Try to fetch and analyze the locations page
        try {
            let locationPageHtml;
            
            // Try to use the existing driver if available
            if (driver) {
                await driver.get(locationPage.url);
                await driver.wait(until.elementLocated(By.tagName('body')), 5000);
                locationPageHtml = await driver.getPageSource();
            } else {
                // Fallback to axios
                const response = await axios.get(locationPage.url, {
                    headers: {
                        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
                    },
                    timeout: 10000
                });
                locationPageHtml = response.data;
            }
            
            const $locationPage = cheerio.load(locationPageHtml);
            const locationCount = finder.extractLocationCount($locationPage);
            const states = finder.extractStateCoverage($locationPage);
            
            return {
                success: true,
                locationPageUrl: locationPage.url,
                locationCount: locationCount,
                states: states,
                confidence: locationPage.confidence
            };
        } catch (error) {
            console.log('[LocationFinder] Error fetching location page:', error.message);
        }
    }
    
    return {
        success: false,
        message: 'Could not find dedicated locations page'
    };
}
