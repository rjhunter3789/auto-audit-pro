...
async function testLocationDirectory($, url) {
    ...
    const selectors = [
        // URL-based selectors (case-insensitive)
        'a[href*="location" i]',
        'a[href*="dealer" i]',
        'a[href*="store" i]',
        'a[href*="showroom" i]',
    ...
    // Check for brand names in links (more comprehensive)
    $('a').each((i, elem) => {
        ...
        const hasBrandName = ALL_BRAND_VARIATIONS.some(brand => 
            text.includes(brand.toLowerCase())
        );
        
        // Check for address patterns (city names, street indicators)
        const hasAddressPattern = /\b(street|st|avenue|ave|road|rd|boulevard|blvd|highway|hwy|drive|dr)\b/i.test(text) ||
                                 /\b(north|south|east|west|downtown|uptown)\b/i.test(text);
        
        if (href && (hasLocationPattern || hasBrandName || hasAddressPattern)) {
            foundUrls.add(href);
