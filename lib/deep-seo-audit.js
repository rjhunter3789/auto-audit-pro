...
class DeepSEOAuditor {
    ...
    async fetchPageDataWithSelenium(url) {
        ...
        try {
            ...
            try {
                await driver.wait(until.elementLocated(By.css('nav, .nav, .navigation, header')), 5000);
                // Wait for actual links to appear (Vue/React apps often load these dynamically)
                await driver.wait(until.elementLocated(By.css('a[href]')), 5000);
                console.log('[Selenium] Navigation and links detected');
                
                // Extra wait for Vue/React to finish rendering
                await driver.sleep(2000);
            } catch (e) {
                console.log('[Selenium] Warning: Navigation or links not found within timeout');
            }
            ...
            if ($('a[href]').length === 0) {
                console.log('[Selenium] No links in HTML, waiting longer for JavaScript rendering...');
                await driver.sleep(3000);
