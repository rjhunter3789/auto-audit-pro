...
class DeepSEOAuditor {
    ...
    async analyzeTechnicalSEO(url, pageData) {
        ...
        try {
            ...
            if (sitemapAnalysis.passed) {
                score += 1;
            } else if (sitemapAnalysis.issue) {
                issues.push(sitemapAnalysis.issue);
            }
            ...
            if (robotsAnalysis.passed) {
                score += 1;
            } else if (robotsAnalysis.issue) {
                issues.push(robotsAnalysis.issue);
            }
            ...
            if (httpsAnalysis.passed) {
                score += 1;
            } else if (httpsAnalysis.issue) {
                issues.push(httpsAnalysis.issue);
            }
    ...
    async fetchPageDataWithSelenium(url) {
        ...
        try {
            ...
            // Additional wait for dynamic content and navigation to load
            await driver.sleep(3000);
            
            // Try to wait for navigation elements and links to load
            try {
                // Wait for common dealership navigation patterns
                await driver.wait(async () => {
                    const linkCount = await driver.findElements(By.css('a[href]')).then(links => links.length);
                    console.log(`[Selenium] Current link count: ${linkCount}`);
                    return linkCount > 20; // Dealership sites typically have many links
                }, 10000, 'Waiting for navigation links to load');
                
                console.log('[Selenium] Navigation links detected');
                
                // Extra wait for all links to stabilize
                await driver.sleep(2000);
            } catch (e) {
                console.log('[Selenium] Warning: Navigation links did not load within timeout');
            ...
            // If no links found, try one more time with JavaScript execution
            const $ = cheerio.load(html);
            const totalLinks = $('a[href]').length;
            const initialLinkCount = $('a[href]:not([href="#"]):not([href=""])').length;
            console.log(`[Selenium] Total links: ${totalLinks}, Non-anchor links: ${initialLinkCount}`);
            
            if (initialLinkCount < 5) {
                console.log(`[Selenium] Only ${initialLinkCount} links found, attempting recovery...`);
                
                // Try multiple strategies to trigger Vue.js rendering
                try {
                    // 1. Try clicking on body to trigger any click handlers
                    await driver.findElement(By.css('body')).click().catch(() => {});
                    await driver.sleep(1000);
                    
                    // 2. Try hovering over common navigation areas
                    const actions = driver.actions();
                    const header = await driver.findElement(By.css('header, .header, [class*="header"]')).catch(() => null);
                    if (header) {
                        await actions.move({origin: header}).perform().catch(() => {});
                        await driver.sleep(1000);
                    }
                    
                    // 3. Execute JavaScript to check for Vue instance
                    await driver.executeScript(`
                        if (window.Vue || window.__VUE__) {
                            console.log('Vue detected, attempting to trigger render');
                            // Try to trigger Vue updates
                    ...
                    console.log('[Selenium] Recovery attempt error:', e.message);
                }
                
                // Get page source again
                const retryHtml = await driver.getPageSource();
                const retry$ = cheerio.load(retryHtml);
                const finalLinkCount = retry$('a[href]:not([href="#"]):not([href=""])').length;
                console.log(`[Selenium] Final link count after recovery: ${finalLinkCount}`);
                ...
                return {
                    ...
                    headers: {},
                    status: 200,
                    method: 'selenium-extended'
                };
            }
            
            console.log(`[Selenium] Successfully loaded ${url} - Title: ${title} - Links: ${$('a[href]').length}`);
    ...
    async crawlSite(startUrl, $, depth) {
            ...
            if (href) {
                try {
                    ...
                    if (linkUrl.hostname === baseUrl.hostname) {
                        ...
                        // Skip if it's the same page (self-link) or already crawled
                        if (normalizedUrl !== normalizedStartUrl && !this.crawledUrls.has(normalizedUrl)) {
                            // Skip if it's just an anchor link to the same page
                            if (!href.startsWith('#') && linkUrl.pathname !== '#') {
        ...
        if (internalLinks.length > 0 && internalLinks.length < 10) {
            console.log('[Crawler] Links found:', internalLinks.slice(0, 5));
        }
        
        // Debug: Log link detection issues
        if (internalLinks.length === 0) {
            const allLinks = $('a').length;
            console.log(`[Crawler Debug] Total <a> tags found: ${allLinks}`);
            if (allLinks > 0) {
                const sampleHrefs = $('a').slice(0, 10).map((i, el) => $(el).attr('href')).get();
                console.log('[Crawler Debug] Sample hrefs:', sampleHrefs);
                // Count how many are self-links
                const selfLinks = sampleHrefs.filter(href => {
                    if (!href) return false;
                    try {
                        const linkUrl = new URL(href, startUrl);
                        return this.normalizeUrl(linkUrl.href) === this.normalizeUrl(startUrl);
                    } catch (e) {
                        return false;
                    }
                }).length;
                console.log(`[Crawler Debug] ${selfLinks} of these are self-links to current page`);
            }
            // Check if the page might be using JavaScript routing
            const jsFrameworks = ['react', 'angular', 'vue', 'next', 'gatsby'];
            const pageHtml = $.html().toLowerCase();
            jsFrameworks.forEach(fw => {
                if (pageHtml.includes(fw)) {
                    console.log(`[Crawler Debug] Detected possible ${fw} app - may need more wait time`);
                }
            });
        }
     ...
     */
    /**
     * Helper to process test results and collect issues
     */
    processTestResult(test, tests, issues, score) {
        tests.push(test);
        
        if (test.passed) {
            return score + 1;
        } else if (test.issue) {
            issues.push(test.issue);
        }
    ...
    compileIssuesAndOpportunities(results) {
        for (const [categoryName, categoryData] of Object.entries(results.categories)) {
            // First, check if there's an issues array
            ...
            // Also check individual test results for issues
            if (categoryData.tests) {
                categoryData.tests.forEach(test => {
                    if (!test.passed && test.issue) {
                        const issueWithCategory = { ...test.issue, category: categoryName, test: test.name };
                        
                        if (test.issue.priority === 'critical' || test.issue.priority === 'high') {
                            results.criticalIssues.push(issueWithCategory);
                        } else if (test.issue.priority === 'medium') {
                            results.warnings.push(issueWithCategory);
                        } else {
                            results.opportunities.push(issueWithCategory);
                        }
    ...
    analyzeMetaDescriptions($) {
        ...
        return {
            ...
            issue: !hasDesc ? {
                ...
                details: 'Your page has no meta description. This is the text that appears below your title in search results.',
                recommendation: 'Add a compelling meta description between 120-160 characters that summarizes your page content and includes relevant keywords.',
                priority: 'high'
            } : (!isOptimalLength ? {
                title: length < 120 ? 'Meta Description Too Short' : 'Meta Description Too Long',
                details: `Your meta description is ${length} characters. Search engines typically display 120-160 characters. ${length > 160 ? 'The excess text will be cut off with "..." in search results.' : 'A longer description provides more opportunity to convince users to click.'}`,
                recommendation: length > 160 ? 
                    'Shorten your meta description to under 160 characters while keeping the most important information.' : 
                    'Expand your meta description to at least 120 characters to better describe your page content.',
                priority: 'medium'
            } : null)
        };
    }
    ...
    analyzeOpenGraphTags($) {
        ...
        return {
            ...
            issue: !hasOG ? {
                ...
                details: `Open Graph tags control how your dealership appears when shared on Facebook, LinkedIn, and other social platforms. Missing tags: ${missingTags.join(', ')}`,
                recommendation: 'Add Open Graph meta tags to your HTML head: og:title (dealership name), og:description (compelling description), og:image (dealership logo or showroom photo), and og:type (website).',
                priority: 'medium'
            } : null
        };
    }
    ...
    analyzeLCP(psiData) {
        if (!psiData) return { 
            ...
            issue: {
                title: 'Unable to Measure Largest Contentful Paint (LCP)',
                details: 'Core Web Vitals could not be measured. LCP shows how fast the main content of your page loads for users.',
                recommendation: 'LCP measures loading performance. To improve it: optimize images, implement lazy loading, use a CDN, minimize CSS/JS blocking time. Target: under 2.5 seconds for good user experience.',
                priority: 'medium'
            }
        };
    ...
    analyzeFID(psiData) {
        if (!psiData) return { 
            ...
            value: 'Unable to measure',
            issue: {
                title: 'Unable to Measure First Input Delay (FID)',
                details: 'Core Web Vitals could not be measured. FID shows how quickly your site responds when users click or tap.',
                recommendation: 'FID measures interactivity. To improve it: break up long JavaScript tasks, use web workers, defer non-critical JS, reduce third-party script impact. Target: under 100ms for good user experience.',
                priority: 'medium'
            }
        };
    ...
    analyzeCLS(psiData) {
        if (!psiData) return { 
            ...
            value: 'Unable to measure',
            issue: {
                title: 'Unable to Measure Cumulative Layout Shift (CLS)',
                details: 'Core Web Vitals could not be measured. CLS shows how much page content unexpectedly shifts during loading.',
                recommendation: 'CLS measures visual stability. To improve it: always include size attributes on images/videos, avoid inserting content above existing content, use CSS transform for animations. Target: under 0.1 for good user experience.',
                priority: 'medium'
            }
        };
