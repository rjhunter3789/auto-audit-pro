...
const { By, until } = seleniumWrapper.seleniumAvailable ? require('selenium-webdriver') : { By: null, until: null };
...
class DeepSEOAuditor {
    ...
    async fetchPageData(url) {
        try {
            const response = await axios.get(url, {
                timeout: 10000,
                headers: {
                    'User-Agent': 'Mozilla/5.0 (compatible; AutoAuditPro/2.6.3; +https://autoauditpro.io)'
                }
            });
    ...
    async crawlSite(startUrl, $, depth) {
        ...
        $('a[href]').each((i, el) => {
            ...
            if (href) {
                try {
                    ...
                    if (linkUrl.hostname === baseUrl.hostname && !this.crawledUrls.has(linkUrl.href)) {
        ...
        // Add to crawled URLs
        this.crawledUrls.add(startUrl);
        this.internalLinks.set(startUrl, internalLinks);

        // Crawl internal links (limited for performance)
        const linksToCrawl = internalLinks.slice(0, 5);
        for (const link of linksToCrawl) {
            if (this.crawledUrls.size >= this.maxUrlsToCrawl) break;
    ...
    checkOrphanedPages() {
        ...
        return { 
            ...
            issue: orphaned.length > 0 ? {
                ...
                details: `Found ${orphaned.length} pages with no internal links pointing to them.`,
                priority: 'medium'
            } : null
        };
    }
