...
class ScrapingDogWrapper {
    constructor(apiKey) {
        ...
        this.statsFile = path.join(__dirname, '../data/monitoring/scrapingdog-stats.json');
        ...
        Object.defineProperty(this, 'apiKey', {
            get: function() {
                return this._providedApiKey || process.env.SCRAPINGDOG_API_KEY;
            }
        });
        
        // Debug logging
        console.log('[ScrapingDog] Initializing - API key will be loaded on demand');
    }
    ...
    async scrape(url, options = {}) {
        ...
        try {
            ...
            const params = {
                api_key: this.apiKey,
                url: url,
                dynamic: options.javascript !== false ? 'true' : 'false', // Convert to string
                premium: options.premium ? 'true' : 'false', // Convert to string
            ...
            if (response.status === 200) {
                ...
                this.saveStats(); // Save stats after failure
                console.error(`[ScrapingDog] Failed: ${url} - Status ${response.status}`);
                console.error(`[ScrapingDog] Response data:`, response.data);
                console.error(`[ScrapingDog] Response headers:`, response.headers);
                
                // Handle specific error codes
                if (response.status === 410) {
                    console.error('[ScrapingDog] ERROR 410: This API endpoint is deprecated. Please check ScrapingDog documentation.');
                } else if (response.status === 401) {
                    console.error('[ScrapingDog] ERROR 401: Invalid API key');
                } else if (response.status === 429) {
                    console.error('[ScrapingDog] ERROR 429: Rate limit exceeded');
                } else if (response.status === 400) {
                    console.error('[ScrapingDog] ERROR 400: Bad request - check parameters');
                }
                
                return {
                    success: false,
                    error: `ScrapingDog returned status ${response.status}: ${response.data || 'No error message'}`,
            ...
            this.usageStats.failedRequests++;
            this.saveStats(); // Save stats after error
    ...
    async getContent(url, options = {}) {
        console.log(`[ScrapingDog] getContent called for ${url}`);
        console.log(`[ScrapingDog] API Key available: ${!!this.apiKey}`);
    ...
    getUsageStats() {
        return {
            ...
            averageCost: this.usageStats.totalRequests > 0
                ...
                : '$0',
            lastUpdated: this.usageStats.lastUpdated || new Date().toISOString()
        };
    }
    ...
    loadStats() {
        try {
            ...
            console.log('[ScrapingDog] Could not load stats:', error.message);
        }
        return null;
    }
    ...
    saveStats() {
        try {
            ...
            fs.writeFileSync(this.statsFile, JSON.stringify(this.usageStats, null, 2));
        } catch (error) {
            console.error('[ScrapingDog] Could not save stats:', error);
        }
    }
