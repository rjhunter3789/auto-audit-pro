...
class JSONStorage {
    ...
    async safeWriteFile(filePath, data) {
        ...
        try {
            ...
            console.log(`[JSON Storage] Safely wrote ${filePath}`);
        } catch (error) {
            console.error(`[JSON Storage] Failed to write ${filePath}:`, error);
            
            // Try to restore from backup
            try {
                await fs.copyFile(backupPath, filePath);
                console.log(`[JSON Storage] Restored ${filePath} from backup`);
            } catch (e) {
                console.error(`[JSON Storage] Failed to restore backup:`, e);
            }
    ...
    // ===== PROFILES =====
    async getProfiles() {
        return await fileLockManager.readJSONWithLock(this.profilesFile) || [];
    }
    ...
    async getApprovedProfiles() {
        const profiles = await this.getProfiles();
        return profiles.filter(p => p.status === 'approved' || !p.status); // Include old profiles without status
    }
    ...
    async createProfile(profile) {
        return await fileLockManager.updateJSONWithLock(this.profilesFile, (profiles) => {
            const newProfile = {
                id: Date.now(),
                ...profile,
                monitoring_enabled: true,
                status: profile.status || 'pending', // New: pending, approved, denied
                requested_by: profile.requested_by || null,
                approved_by: profile.approved_by || null,
                approval_date: profile.approval_date || null,
    ...
    async deleteProfile(id) {
        ...
        await fileLockManager.updateJSONWithLock(this.profilesFile, (profiles) => {
            return profiles.filter(p => p.id != id);
        });
        
        // Delete associated alerts
        await fileLockManager.updateJSONWithLock(this.alertsFile, (alerts) => {
            return alerts.filter(a => a.profile_id != id);
        });
        
        // Delete associated results
        await fileLockManager.updateJSONWithLock(this.resultsFile, (results) => {
            return results.filter(r => r.profile_id != id);
        });
    ...
    // ===== RESULTS =====
    async getResults(profileId = null, limit = 100) {
        const allResults = await fileLockManager.readJSONWithLock(this.resultsFile) || [];
    ...
    async saveResult(result) {
        return await fileLockManager.updateJSONWithLock(this.resultsFile, (results) => {
            const newResult = {
                id: Date.now(),
                ...result,
    ...
    async getAlerts(profileId, resolved = false) {
        ...
        // Convert profileId to number for comparison since it may come as string
        const numericProfileId = parseInt(profileId);
        
        // If profileId is not provided, return all alerts matching resolved status
        if (!profileId) {
            return allAlerts.filter(a => a.resolved === resolved);
        }
        
        return allAlerts
            .filter(a => a.profile_id == numericProfileId && a.resolved === resolved)
    ...
    async saveAlert(alert) {
        ...
        await this.safeWriteFile(this.alertsFile, alerts);
    ...
    query: async (query, values) => {
        ...
        if (query.includes('monitoring_profiles')) {
            if (query.includes('INSERT')) {
                ...
                const id = values[0];
                // Handle different UPDATE queries
                let updates = {};
                if (query.includes('monitoring_enabled')) {
                    updates = { monitoring_enabled: values[1] };
                } else if (query.includes('updated_at')) {
                    // Just updating timestamp, no other changes needed
                    updates = {};
                }
            ...
            if (query.includes('INSERT')) {
                ...
                const resolved = values[1] === 'true' || values[1] === true;
