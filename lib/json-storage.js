...
class JSONStorage {
     ...
     * IMPROVED: Only keeps backup of successfully written data
     */
    async safeWriteFile(filePath, data) {
        ...
        try {
            ...
            // 3. IMPORTANT: Update backup AFTER successful write
            // This ensures backup always reflects the last successful state
            try {
                await fs.copyFile(filePath, backupPath);
                console.log(`[JSON Storage] Successfully wrote ${filePath} and updated backup`);
            } catch (backupError) {
                // Backup update failed, but main write succeeded
                console.error(`[JSON Storage] Main write succeeded but backup update failed:`, backupError);
                // This is not critical - main operation succeeded
            }
    ...
    async createProfile(profile) {
        ...
        await this.safeWriteFile(this.profilesFile, profiles);
    ...
    async deleteProfile(id) {
        ...
        // Safe write with backup
        await this.safeWriteFile(this.profilesFile, updatedProfiles);
        ...
        await this.safeWriteFile(this.alertsFile, updatedAlerts);
        await this.safeWriteFile(this.resultsFile, updatedResults);
    ...
    async saveResult(result) {
        ...
        // Safe write with backup
        await this.safeWriteFile(this.resultsFile, results);
